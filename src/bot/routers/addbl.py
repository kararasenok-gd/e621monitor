...
@router.message(Command("addbl"))
async def start_handler(message: Message, session):
    lang = (await check_exists(session, message.from_user)).lang
    args = get_args(message)

    if not args:
        return await message.answer(f"""<tg-emoji emoji-id="5210952531676504517">❌</tg-emoji> {get_translation('addbl.missing_args', lang)}

{get_translation('addbl.usage', lang)}""")

    plur = pluralize(min(len(args.split(" ")), 2), "one", "few", "few", "few")

    tags = get_translation('add.tags.'  + plur, lang)
    succ = get_translation('addbl.success.' + plur, lang)
